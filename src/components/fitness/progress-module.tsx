...
export default function ProgressModule({ userId }: ProgressModuleProps) {
  ...
  const fetchProgressData = async () => {
    try {
      ...
      // Gerar feedback motivacional baseado nos dados
      generateMotivationalFeedback(metrics)
  ...
  const generateMotivationalFeedback = (metrics: any) => {
    if (!metrics) {
      setMotivationalFeedback("Continue registrando seus dados para receber feedback personalizado!")
      return
    }
    ...
    const feedbacks = [
      `🎉 Incrível! Você perdeu ${Math.abs(weightLost).toFixed(1)}kg e reduziu ${Math.abs(fatReduced).toFixed(1)}% de gordura este mês. Sua dedicação está dando resultados!`,
      `💪 Parabéns! Você ganhou ${muscleGained.toFixed(1)}kg de massa muscular. Continue com o treino de força e alimentação adequada!`,
      `🔥 Excelente progresso! Sua composição corporal melhorou significativamente. Você está ${Math.abs(weightLost).toFixed(1)}kg mais leve e mais forte!`,
      `⭐ Fantástico! Você reduziu ${Math.abs(fatReduced).toFixed(1)}% de gordura corporal. Sua definição muscular está aumentando visivelmente!`,
      `🚀 Você está arrasando! ${Math.abs(weightLost).toFixed(1)}kg perdidos e ${muscleGained.toFixed(1)}kg de músculo ganho. Continue assim!`,
      `🎯 Meta atingida! Sua evolução este mês foi excepcional. Você está no caminho certo para seus objetivos!`,
      `💎 Transformação incrível! Seus números mostram dedicação e consistência. Continue focado nos seus treinos!`,
      `🌟 Progresso consistente! Você perdeu ${Math.abs(weightLost).toFixed(1)}kg mantendo a massa muscular. Perfeito!`,
      `🏆 Resultados impressionantes! Sua gordura corporal caiu ${Math.abs(fatReduced).toFixed(1)}% enquanto ganhou ${muscleGained.toFixed(1)}kg de músculo!`,
      `✨ Evolução notável! Você está ${Math.abs(weightLost).toFixed(1)}kg mais leve e sua definição muscular melhorou muito!`,
      `🎊 Parabéns pela consistência! Seus resultados este mês mostram que você está no caminho certo!`,
      `💯 Excelente trabalho! Você reduziu ${Math.abs(fatReduced).toFixed(1)}% de gordura e ganhou força. Continue assim!`
    ]

    // Selecionar feedback baseado no mês atual (muda mensalmente)
    const feedbackIndex = (currentMonth + currentYear) % feedbacks.length
    setMotivationalFeedback(feedbacks[feedbackIndex])
  }
